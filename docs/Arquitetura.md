...
- `src/lib/firebase.ts`: inicialização condicional do Firebase (Auth).
- `firebase/DATA-MODEL.md`: modelo de dados Firestore, seeds e automações.
- `firebase/firestore.rules` e `firebase/storage.rules`: regras de segurança.
...
O fallback local usa `localStorage`. Regras que exigem consistência multiusuário devem migrar gradualmente para serviços Firebase (Firestore/Cloud Functions).
...
- Nunca usar credenciais de service account (Admin SDK) ou Google client secret no frontend.
...
- As Security Rules do Firestore/Storage devem ser revisadas antes da produção.

## Próxima evolução arquitetural

1. Criar serviços por agregado em `src/services`.
2. Persistir o ciclo do Livro e metadados do Drive no Firestore.
